...
import type { Lead, Visitor, Client } from '../types'
...
export default function AdminDashboard() {
  ...
  const [activeSection, setActiveSection] = useState<'dashboard' | 'leads' | 'visitors' | 'clients'>('dashboard')
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-500">
                    ...
                    {activeSection === 'clients' && 'Manage and track all your clients'}
                  </p>
                </div>
              </div>
...
function ClientDetailsSection({ clients, onClientsChange, loading }: ClientDetailsSectionProps) {
  ...
  const handleCellEdit = async (clientId: string, field: string, value: any) => {
    ...
    try {
      ...
      onClientsChange(clients.map(c => c.id === clientId ? updatedClient : c))
      setEditingCell(null)
    } catch (err) {
  ...
  const renderCell = (client: Client, field: string) => {
    ...
    if (['first_call', 'first_meeting', 'agreement_signed', 'project_delivered'].includes(field)) {
      return (
        <button
          ...
          className={`px-2 py-1 md:px-3 md:py-2 rounded font-medium text-xs md:text-sm transition-colors ${
            value
    ...
    if (isEditing) {
      return (
        <input
          ...
          autoFocus
          disabled={saving}
          className="w-full px-1.5 py-0.5 md:px-2 md:py-1 border border-blue-500 rounded text-xs md:text-sm"
        />
      )
    }
    ...
    return (
      <button
        ...
        className="text-left hover:bg-gray-50 px-1 md:px-2 py-0.5 md:py-1 rounded transition-colors text-xs md:text-sm"
        disabled={saving}
      >
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
        <div>
          <h2 className="text-lg md:text-2xl font-bold text-[#0B132B]">Manage Clients</h2>
          <p className="text-xs md:text-sm text-slate-600 mt-1">Manage and track all your clients</p>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="flex items-center justify-center gap-2 px-3 md:px-4 py-2 md:py-2.5 bg-[#EAB308] text-[#0B132B] hover:bg-[#EAB308]/90 font-semibold rounded-lg md:rounded-xl transition-colors w-full md:w-auto text-sm"
        >
      ...
      <div className="mb-4 md:mb-6">
        <input
          ...
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full px-3 md:px-4 py-2 md:py-2.5 rounded-lg md:rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#EAB308]/30 outline-none text-xs md:text-sm"
        />
      </div>
      ...
      <div className="bg-white rounded-lg md:rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-48 md:h-64">
            <Loader className="w-6 h-6 md:w-8 md:h-8 animate-spin text-[#EAB308]" />
          </div>
        ) : filteredClients.length === 0 ? (
          <div className="flex items-center justify-center h-48 md:h-64 text-slate-500 text-sm md:text-base">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-slate-100 border-b border-slate-200">
                <tr>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-slate-700 whitespace-nowrap">Sr</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-slate-700 whitespace-nowrap">Business</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-slate-700 whitespace-nowrap">Owner</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-slate-700 whitespace-nowrap">Contact</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-slate-700 whitespace-nowrap">Address</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-center font-semibold text-slate-700 whitespace-nowrap">Call</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-center font-semibold text-slate-700 whitespace-nowrap">Meet</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-center font-semibold text-slate-700 whitespace-nowrap">Agree</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-right font-semibold text-slate-700 whitespace-nowrap">Amount</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-right font-semibold text-slate-700 whitespace-nowrap">Recv</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-center font-semibold text-slate-700 whitespace-nowrap">Deliv</th>
                  <th className="px-2 md:px-4 py-2 md:py-3 text-center font-semibold text-slate-700 whitespace-nowrap">Delete</th>
                </tr>
              </thead>
              <tbody>
                {filteredClients.map((client, index) => (
                  <tr key={client.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap text-slate-600 text-xs md:text-sm">{index + 1}</td>
                    <td className="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap">
                      {renderCell(client, 'business_name')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap">
                      {renderCell(client, 'owner_name')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap text-slate-700 text-xs md:text-sm">
                      {client.owner_contact_number || '-'}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap">
                      {renderCell(client, 'address_name')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {renderCell(client, 'first_call')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {renderCell(client, 'first_meeting')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {renderCell(client, 'agreement_signed')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-right">
                      {renderCell(client, 'payment_amount')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-right">
                      {renderCell(client, 'amount_received')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {renderCell(client, 'project_delivered')}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      <button
                        onClick={() => handleDeleteClient(client.id)}
                        className="text-red-600 hover:text-red-800 transition-colors disabled:opacity-50 text-xs md:text-sm"
                        disabled={saving}
                      >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg md:rounded-xl shadow-xl max-w-sm w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-200 p-4 md:p-6 flex items-center justify-between">
              <h2 className="text-lg md:text-xl font-bold text-slate-900">Create New Client</h2>
              <button
            ...
            <form onSubmit={handleCreateClient} className="p-4 md:p-6 space-y-3 md:space-y-4">
              <div>
                <label className="block text-xs md:text-sm font-medium text-slate-700 mb-1">Business Name *</label>
                <input
                  type="text"
                  name="business_name"
                  ...
                  required
                  className="w-full px-2 md:px-3 py-1.5 md:py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#EAB308] outline-none text-xs md:text-sm"
                  placeholder="Enter business name"
                />
              </div>

              <div>
                <label className="block text-xs md:text-sm font-medium text-slate-700 mb-1">Owner Name</label>
                <input
                  type="text"
                  name="owner_name"
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-slate-700 mb-1">Contact Number</label>
                <input
                  type="tel"
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-slate-700 mb-1">Address Name</label>
                <input
                  type="text"
                  name="address_name"
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-slate-700 mb-1">Google Maps Link</label>
                <input
                  type="url"
              ...
              <div className="flex gap-2 md:gap-3 pt-2 md:pt-4">
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="flex-1 px-3 md:px-4 py-1.5 md:py-2 border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 transition-colors text-xs md:text-sm"
                  disabled={saving}
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 px-3 md:px-4 py-1.5 md:py-2 bg-[#EAB308] text-[#0B132B] rounded-lg hover:bg-[#EAB308]/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-xs md:text-sm"
                  disabled={saving}
                >
                  {saving ? (
                    <>
